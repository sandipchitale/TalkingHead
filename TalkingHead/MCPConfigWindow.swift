...
// The "MCP Server Config…" menu item, as in VoiceChat: a sample client configuration to copy or
// save instead of hand-assembling paths and ports, either as a config file or as the commands that
// register Talking Head from a shell.
...
enum MCPConfigFormat: String, CaseIterable, Identifiable {
    ...
    var headline: String {
        ...
        case .shell: "Or register Talking Head from a terminal"
        }
    }
    ...
    var detail: String {
        switch self {
        case .json:
            "For example a project's .mcp.json. Use either entry. The HTTP one also needs “MCP Server” turned on in the menu bar."
        case .shell:
            "Each line removes any earlier entry, then adds it again, so it is safe to re-run. The HTTP entries also need “MCP Server” turned on in the menu bar."
        }
    }
...
enum MCPConfigSample {
    static let stdioPath = "/Applications/TalkingHead.app/Contents/MacOS/th-mcp"
    static var httpURL: String {
        "http://127.0.0.1:\(MCPServerController.configuredPort)/mcp"
    }
    ...
    static func shellCommands() -> [ShellCommand] {
        ...
        return [
            .init(host: "Claude Code", transport: .stdio,
                  command: "claude mcp remove talkinghead-stdio ; claude mcp add talkinghead-stdio \(path)"),
            .init(host: "Claude Code", transport: .http,
                  command: "claude mcp remove talkinghead-http ; claude mcp add --transport http talkinghead-http \(url)"),
            .init(host: "Antigravity", transport: .stdio,
                  command: "agy mcp remove talkinghead-stdio ; agy mcp add talkinghead-stdio \(path)"),
            .init(host: "Antigravity", transport: .http,
                  command: "agy mcp remove talkinghead-http ; agy mcp add talkinghead-http \(url)"),
            .init(host: "Codex", transport: .stdio,
                  command: "codex mcp remove talkinghead-stdio ; codex mcp add talkinghead-stdio -- \(path)"),
            .init(host: "Codex", transport: .http,
                  command: "codex mcp remove talkinghead-http ; codex mcp add talkinghead-http --url \(url)"),
        ]
    }
...
final class MCPConfigWindowController: NSWindowController {
    ...
    private init() {
        ...
        // The face floats while it speaks; this dialog must not hide behind it.
